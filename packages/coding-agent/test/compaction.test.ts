...
describe("prepareCompaction with previous compaction", () => {
	...
	it("uses only the latest summary in the active source prefix", () => {
		...
		const preparation = prepareCompaction([u1, a1, u2, a2, compaction1, u3, a3, compaction2, u4, a4], settings);
		...
		expect(extractText(sourceSummaries)).toBe("Second summary");
		expect(extractText(preparation.sourceMessages)).not.toContain("First summary");
	});
