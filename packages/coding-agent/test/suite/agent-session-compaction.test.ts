...
describe("AgentSession compaction characterization", () => {
	...
	it("uses the standalone compaction context outside experimental mode", async () => {
	...
	it.each([
		...
		expect(convertedInputs).toHaveLength(testCase.expectedCalls + (preparation.isSplitTurn ? 1 : 0));

		for (let i = 0; i < sourcePrefixes.length; i++) {
			const transformedSource = [transformMarker, ...sourcePrefixes[i]];
			expect(convertedInputs[i]).toEqual(transformedSource);
	...
	it("uses standalone split-turn summarization when a context transform appends a message", async () => {
		process.env.PI_EXPERIMENTAL = "1";
		const harness = await createHarness({ settings: { compaction: { keepRecentTokens: 1 } } });
		harnesses.push(harness);
		...
		expect(requestOptions[0]).toMatchObject({ cacheRetention: "short" });
		expect(requestContexts[1].systemPrompt).not.toBe(harness.session.agent.state.systemPrompt);
		expect(requestContexts[1].tools).toBeUndefined();
		expect(JSON.stringify(requestContexts[1].messages)).toContain("<conversation>");
		expect(JSON.stringify(requestContexts[1].messages)).not.toContain("provider-only context");
		expect(requestOptions[1]).toMatchObject({ cacheRetention: "none" });
		expect(requestOptions[1]?.sessionId).not.toBe(harness.session.agent.sessionId);
	});
